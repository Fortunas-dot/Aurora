...
export default function CreatePostScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // #region agent log
      if (response.success && response.data) {
        fetch('http://127.0.0.1:7244/ingest/083d67a2-e9cc-407e-8327-24cf6b490b99',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'create-post.tsx:270',message:'Post created - URLs in response',data:{postId:response.data._id, images:response.data.images, video:response.data.video, imagesAreAbsolute:response.data.images?.map((u: string) => u?.startsWith('http')), videoIsAbsolute:response.data.video?.startsWith('http')},timestamp:Date.now(),runId:'run1',hypothesisId:'A'})}).catch(()=>{});
      }
      // #endregion
