...
export const seedDatabase = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const createdPosts = [];
    const postCommentsById = new Map<string, string[]>();
    for (let i = 0; i < samplePosts.length; i++) {
      ...
      if (Array.isArray(postComments) && postComments.length > 0) {
        postCommentsById.set(post._id.toString(), postComments);
      }
    }
    ...
    for (let i = 0; i < createdPosts.length; i++) {
      ...
      const postComments = postCommentsById.get(post._id.toString());

      // If we have post specific comments, create them deterministically (no randomness).
      if (Array.isArray(postComments) && postComments.length > 0) {
